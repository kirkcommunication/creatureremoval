...
class Module {
	...
	public static function render( array $args ): string {
		...
		// Add custom attributes if present. Preset-sourced CSS id entries are dropped because a
		// shared preset would repeat the same DOM id on every module instance using it.
		// `$attrs` already includes ModuleRegistration's preset renderAttrs merge, so provenance
		// is read from the preset APIs rather than inferred from that already-merged array.
		// Collect that provenance only when the payload has an `id` row to evaluate.
		$custom_attributes_data = $ownership_attrs['module']['decoration']['attributes'] ?? [];
		if ( ! empty( $custom_attributes_data ) && ! empty( self::_get_id_attribute_entry_ids( $custom_attributes_data ) ) ) {
			$custom_attributes_data = self::_remove_preset_sourced_id_attributes(
				$custom_attributes_data,
				self::_collect_preset_sourced_id_attribute_entry_ids( $name, $attrs, $selected_group_presets )
			);
		}
	...
	/**
	 * Collect custom-attribute entry ids whose name is `id`.
	 *
	 * @since ??
	 *
	 * @param array $attributes_data Custom attribute data in the decoration.attributes shape.
	 *
	 * @return array<string, true> Entry ids keyed for cheap lookup.
	 */
	private static function _get_id_attribute_entry_ids( array $attributes_data ): array {
		$entry_ids = [];

		foreach ( $attributes_data as $breakpoint_data ) {
			if ( ! is_array( $breakpoint_data ) ) {
				continue;
			}

			foreach ( $breakpoint_data as $state_data ) {
				if ( ! is_array( $state_data ) ) {
					continue;
				}

				foreach ( $state_data['attributes'] ?? [] as $entry ) {
					if ( ! is_array( $entry ) ) {
						continue;
					}

					if ( 'id' === ( $entry['name'] ?? '' ) && isset( $entry['id'] ) ) {
	...
	/**
	 * Collect preset-sourced custom `id` entry ids from the preset planes.
	 *
	 * `Module::render()` receives `$attrs` after ModuleRegistration has already folded
	 * preset `renderAttrs` into the module attrs. Custom attributes are classified as HTML
	 * preset data, so a preset-authored `id` row is already present in that merged array.
	 * Provenance is therefore read from each preset item's own `attrs` and `renderAttrs`
	 * rather than from `get_merged_attrs( presetOnly )`. That merge uses
	 * `array_replace_recursive` and skips mergeable-field concatenation, so a group
	 * preset row at the same `decoration.attributes` index can overwrite a module-preset
	 * `id` entry and omit it from the suppression set.
	 *
	 * @since ??
	 *
	 * @param string $module_name            Module name.
	 * @param array  $module_attrs           Module attrs used to select the preset stack.
	 * @param array  $selected_group_presets Selected group presets for this module.
	 *
	 * @return array<string, true> Preset-sourced `id` entry ids keyed for cheap lookup.
	 */
	private static function _collect_preset_sourced_id_attribute_entry_ids( string $module_name, array $module_attrs, array $selected_group_presets ): array {
		$entry_ids  = [];
		$preset_ids = GlobalPreset::normalize_preset_stack( $module_attrs['modulePreset'] ?? '' );

		if ( empty( $preset_ids ) ) {
			$module_presets = [
				GlobalPreset::get_selected_preset(
					[
			...
			foreach ( $preset_ids as $preset_id ) {
				$module_presets[] = GlobalPreset::get_selected_preset(
					[
						'moduleName'  => $module_name,
						'moduleAttrs' => array_merge(
							$module_attrs,
							[
		...
		foreach ( $module_presets as $module_preset ) {
			$entry_ids += self::_collect_id_attribute_entry_ids_from_preset_item( $module_preset );
		}

		foreach ( $selected_group_presets as $group_preset ) {
			if ( ! ( $group_preset instanceof GlobalPresetItemGroup ) ) {
				continue;
			}

			$entry_ids += self::_collect_id_attribute_entry_ids_from_preset_item( $group_preset );
		}
	...
	/**
	 * Collect custom `id` entry ids from one preset item's attrs and renderAttrs planes.
	 *
	 * @since ??
	 *
	 * @param GlobalPresetItem $preset_item Module or group preset item.
	 *
	 * @return array<string, true> Preset-sourced `id` entry ids keyed for cheap lookup.
	 */
	private static function _collect_id_attribute_entry_ids_from_preset_item( GlobalPresetItem $preset_item ): array {
		if ( ! $preset_item->is_exist() ) {
			return [];
		}

		return self::_get_id_attribute_entry_ids( $preset_item->get_data_attrs()['module']['decoration']['attributes'] ?? [] )
			+ self::_get_id_attribute_entry_ids( $preset_item->get_data_render_attrs()['module']['decoration']['attributes'] ?? [] );
	}

	/**
	 * Drop preset-sourced CSS id entries from custom attribute data.
	 *
	 * Runtime preset migration rewrites a preset's `module.advanced.htmlAttributes` into
	 * `module.decoration.attributes` entries named `id` and `class`. Saved presets also store
	 * Custom Attributes on the HTML `renderAttrs` plane. A preset is shared across module
	 * instances, so keeping its `id` entry would emit the same DOM id more than once.
	 * Locally authored `id` entries stay because they belong to a single module instance.
	 *
	 * @since ??
	 *
	 * @param array $merged_attributes           Custom attribute data resolved through the preset stack.
	 * @param array $preset_sourced_id_entry_ids Preset-sourced `id` entry ids keyed for cheap lookup.
	 *
	 * @return array Custom attribute data without preset-only `id` entries.
	 */
	private static function _remove_preset_sourced_id_attributes( array $merged_attributes, array $preset_sourced_id_entry_ids ): array {
		if ( empty( $merged_attributes ) ) {
			return $merged_attributes;
		}

		foreach ( $merged_attributes as $breakpoint => $breakpoint_data ) {
			if ( ! is_array( $breakpoint_data ) ) {
				continue;
			}

			foreach ( $breakpoint_data as $state => $state_data ) {
				$entries = $state_data['attributes'] ?? null;
				...
				foreach ( $entries as $entry ) {
					$is_id_entry     = 'id' === ( $entry['name'] ?? '' );
					$is_preset_entry = isset( $entry['id'] ) && isset( $preset_sourced_id_entry_ids[ $entry['id'] ] );

					if ( $is_id_entry && $is_preset_entry ) {
						continue;
					}
				...
				$merged_attributes[ $breakpoint ][ $state ]['attributes'] = array_values( $kept_entries );
			}
		}
